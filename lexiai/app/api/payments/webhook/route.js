...
async function getCustomerEmail(customerId) {
    try {
        const customer = await stripe.customers.retrieve(customerId);
        return (customer).email; // as Stripe.Customer
    } catch (error) {
        console.error("Error fetching customer:", error);
        return null;
    }
...
async function handleSubscriptionEvent(
    ...
    try {
        if (type === "deleted") {
            await subscriptionRef.update({ status: "cancelled", email: customerEmail });
            await db.collection('users').doc(customerEmail).update({ subscription: null });
        } else {
...
async function handleCheckoutSessionCompleted(event) { // Stripe.Event
    ...
    if (metadata?.subscription === "true") {
        ...
        try {
            ...
            await db.collection('invoices').where('email', '==', metadata?.email)
                .get().then(snapshot => {
                    snapshot.forEach(doc => {
                        doc.ref.update({ user_id: metadata?.userId });
                    });
                });

            await db.collection('users').doc(metadata?.userId).update({ subscription: session.id });
        ...
        try {
            ...
            const updatedCredits = Number(userData?.credits || 0) + (session.amount_total || 0) / 100;
            await db.collection('users').doc(metadata?.userId).update({ credits: updatedCredits });
